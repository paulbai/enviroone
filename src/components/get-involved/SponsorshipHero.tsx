...
export type SponsorshipHeroProps = {
    heading?: string | null;
    subheading?: string | null;
    image?: HeroImage;
};

export const SponsorshipHero = ({ heading, subheading, image }: SponsorshipHeroProps = {}) => {
    const imgSrc =
        image && image.asset
            ? urlFor(image as any).width(1600).height(900).fit("max").url()
            : "/images/restored-landscape.jpg";
    const imgAlt = (image && image.alt) || "Sponsorship";
        ...
        <section className="relative h-[50vh] min-h-[400px] w-full overflow-hidden flex items-center justify-center">
            ...
            >
                ...
                <p className="text-xl md:text-2xl text-cream/90 font-light max-w-2xl mx-auto">
                    {subheading ?? "Your contribution directly transforms lives. Choose a cause and make a lasting impact today."}
                </p>
            </motion.div>
        </section>
    );
