...
export type VolunteerRolesProps = {
    heading?: string | null;
    intro?: string | null;
    roles?: Role[] | null;
};

const FALLBACK_ROLES: { title: string; description: string; details: string[]; image: string; align: "left" | "right" }[] = [
    {
...
export const VolunteerRoles = ({ roles }: VolunteerRolesProps = {}) => {
    const list =
        roles && roles.length > 0
            ? roles.map((r) => ({
                  ...
                  details: r.tasks ?? [],
                  image: urlFor(r.image as any).width(1200).height(900).fit("max").url(),
                  align: r.align,
              }))
            : FALLBACK_ROLES;
